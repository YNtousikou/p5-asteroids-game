	...
	class Asteroid
		...
		class SpaceShip
		{
			...
			movex(movex)
			{
				if (this.x > 40 && movex < 0) // check that will not get out of the left barrier
				{
					this.x += (movex*10);
				}

				if (this.x < 1160 && movex > 0) // check that will not get out of the right barrier
				{
					this.x += (movex*10);
				}
			}
			...
			movey(movey)
			{
				if (this.y < 420 && movey > 0) // check that will not get out of the bottom barrier
				{
					this.y += (movey*10);
				}

				if (this.y > 0 && movey < 0) // chech that will not get out of the top barrier
				{
					this.y += (movey*10);
				}
			}
		...
		function draw() 
		{
			...
			*/
			...
			if (!gameOver && startGame && !paused) // while the game is played
			{
				...
				if (keyIsDown(36)) // up arrow is pressed
				{
					spaceship.movey(-1);
				}
